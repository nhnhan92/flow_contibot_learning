...
def on_y_positive(state, value):
    """Called when X axis moves positive (right)."""
    print(f"→ Y positive: {value:+.2f}")


def on_y_negative(state, value):
    """Called when X axis moves negative (left)."""
    print(f"← Y negative: {value:+.2f}")
...
dof_callbacks = [
    ...
    pyspacemouse.DofCallback(
        axis="y",
        callback=on_y_positive,
        callback_minus=on_y_negative,
        filter=0.1,  # Deadzone: ignore values < 0.1
        sleep=0.1,  # Rate limit: max 10 calls/second
    ),
...
with pyspacemouse.open(device_index=1 ,dof_callbacks=dof_callbacks) as device:
