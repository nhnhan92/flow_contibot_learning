#!/usr/bin/env python3
"""
collect_flow_tip.py — IK-based data collection for flow proprioception model.

State machine per waypoint:
  MOVE      → fb.step() loop toward random target (not logged)
  SETTLE    → settling_s pause after arrival
  EQUIL_LOG → log equil_s as state="free_human"
  HUMAN_LOG → log deform_s as state="with_human"
  WAIT_NEXT → press Enter (not logged)
  → next waypoint
    ...
    def _run(self):
        while not self._stop.is_set():
            ...
            if not line or not line[0].isdigit():
                continue  # skip ACK / comment lines
            parts = line.split(",")
            if len(parts) < 9:
                continue
            try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect flow + OptiTrack tip data using IK-based random motion."
    )
    parser.add_argument("--port", default=_default_port(),
                        help="Arduino serial port")
    parser.add_argument("--n_waypoints", type=int, default=60,
                        help="Number of random waypoints (default 50)")
    parser.add_argument("--equil_s", type=float, default=0.5,
                        help="Equilibrium logging duration after each step (default 0.5 s)")
    parser.add_argument("--settling_s", type=float, default=2.0,
                        help="Delay after each step before logging (default 0.5 s)")
    parser.add_argument("--deform_s", type=float, default=3.0,
                        help="Human-interaction logging duration after each step (default 3.0 s)")
    parser.add_argument("--output", default=None,
                        help="Output CSV path (default data/flow_tip/collect_<timestamp>.csv)")
    ...
    parser.add_argument("--pwm_min", type=int, default=0,
                        help="Minimum PWM value sent to each actuator (default 1)")
    parser.add_argument("--pwm_max", type=int, default=26,
                        help="Maximum PWM value sent to each actuator (default 25)")
    parser.add_argument("--max_pos_speed", type=float, default=30.0,
    ...
    if args.output is None:
        folder = "data/flow_tip"
        os.makedirs(folder, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        args.output = f"{folder}/seed{args.seed}_{ts}.csv"
    else:
    ...
    # Stop flowbot's internal reader thread so our SerialReader can own the port.
    # The thread exits within one readline timeout (serial timeout=0.1 s).
    fb.stop_flag["stop"] = True
    time.sleep(0.25)
    print("[flowbot] Internal reader stopped; sensor reader starting.")

    # ── Serial reader (sensor data on fb.ser) ─────────────────────────────────
    ...
    # ── Workspace ─────────────────────────────────────────────────────────────
    bbox_lo, bbox_hi = fb.bbox   # (min_xyz, max_xyz) each shape (3,)
    print(f"\n[collect] Workspace bbox:")
    print(f"           lo = {np.round(bbox_lo, 1)} mm")
    print(f"           hi = {np.round(bbox_hi, 1)} mm")

    t0 = time.perf_counter()
    print(f"[collect] Starting {args.n_waypoints} waypoints.")
    ...
    try:
        for wp_idx in range(args.n_waypoints):
            # Sample a random target inside the workspace convex hull (uses our seeded rng)
            for _ in range(10000):
                target = rng.uniform(bbox_lo, bbox_hi)
                if fb.ws.is_inside_workspace(target, fb.tri) and target[2] > bbox_lo[2] + 5.0 and target[2] < bbox_hi[2] - 3.0:
                    break
            else:
            ...
            # ── MOVE: drive toward target at full speed (no logging) ─────────
            max_iters = 20
            step_count = 0
            ...
            # ── HUMAN_LOG: log human interaction as "with_human" ─────────────
            print(f"   Interact now! ({args.deform_s:.1f} s)...")
            n_human = log_timed(writer, f_csv, reader, opti, opti_origin,
                                fb.last_pwm, "with_human", args.deform_s, t0)
            print(f"   → {n_human} rows (with_human)")
            if (wp_idx + 1) % 15 == 0 and wp_idx < args.n_waypoints - 1 and wp_idx > 0:
                print(f"   Intermediate resting point — take a break! ")
                fb.reset()
                time.sleep(30) 
            input("   Press Enter to move to next waypoint... ")
