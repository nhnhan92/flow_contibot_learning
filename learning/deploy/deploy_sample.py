...
class RobotDeployment:
    ...
    def run_episode(self, max_steps=250,
                    enable_gripper_termination=True,
                    enable_movement_termination=False,
                    gripper_close_threshold=0.5,
                    movement_threshold=0.005,  # 5mm
                    movement_window=10):  # Check over 10 steps
        """Run one episode of deployment with intelligent termination

        Args:
            max_steps: Maximum steps (hard limit timeout)
            enable_gripper_termination: Detect pick-place completion via gripper sequence
            enable_movement_termination: Detect task completion via low movement
            gripper_close_threshold: Threshold for gripper closed state (default 0.5)
            movement_threshold: Movement threshold in meters (default 5mm)
            movement_window: Number of steps to check for low movement
        """
        print("\n" + "="*60)
        ...
        self.log(f"Termination detection:")
        self.log(f"  Max steps: {max_steps}")
        if enable_gripper_termination:
            self.log(f"  ✅ Gripper-based termination enabled (threshold={gripper_close_threshold:.2f})")
        if enable_movement_termination:
            self.log(f"  ✅ Movement-based termination enabled (threshold={movement_threshold*100:.1f}cm over {movement_window} steps)")
        self.log(f"")
        ...
        gripper_state_history = []  # Track gripper open/close sequence
        has_grasped = False  # Whether gripper has closed (grasped object)
        has_released = False  # Whether gripper opened again after grasp
        ...
        try:
            while step < max_steps:
                ...
                for i, action in enumerate(actions_to_execute):
                    ...
                    # 1. Gripper-based termination (detect pick-place completion)
                    if enable_gripper_termination:
                        gripper_closed = current_gripper < gripper_close_threshold
                        gripper_state_history.append(gripper_closed)

                        # Detect grasp (gripper closes)
                        if not has_grasped and gripper_closed:
                            has_grasped = True
                            if self.verbose:
                                print(f"  🤏 Gripper CLOSED at step {step} (grasped object)")

                        # Detect release (gripper opens after grasp)
                        if has_grasped and not has_released and not gripper_closed:
                            # Gripper opened after closing - task likely complete!
                            # Wait a few more steps to confirm
                            steps_since_release = 0
                            for i in range(min(5, len(gripper_state_history))):
                                if not gripper_state_history[-(i+1)]:
                                    steps_since_release += 1

                            if steps_since_release >= 3:  # Gripper open for 3+ steps
                                has_released = True
                                termination_reason = f"gripper_sequence (step {step})"
                                if self.verbose:
                                    print(f"\n✅ Task complete! Detected pick-place sequence:")
                                    print(f"   Gripper: OPEN → CLOSE (grasp) → OPEN (release)")
                                self.log(f"\n[Step {step}] ✅ TASK COMPLETED - Gripper sequence detected")
                                break

                    # 2. Movement-based termination (detect robot stopped moving)
                    if enable_movement_termination and len(trajectory_positions) >= movement_window:
                        recent_positions = trajectory_positions[-movement_window:]
                        movements = [np.linalg.norm(recent_positions[i+1] - recent_positions[i])
                                     for i in range(len(recent_positions)-1)]
                        avg_movement = np.mean(movements)

                        if avg_movement < movement_threshold:
                            termination_reason = f"low_movement (avg={avg_movement*100:.2f}cm over {movement_window} steps)"
                            if self.verbose:
                                print(f"\n✅ Task complete! Robot stopped moving (avg movement: {avg_movement*100:.2f}cm)")
                            self.log(f"\n[Step {step}] ✅ TASK COMPLETED - Low movement detected")
                            break
            ...
            if termination_reason:
                self.log(f"✅ Early termination: {termination_reason}")
            else:
                self.log(f"⏱️  Reached max_steps limit ({max_steps})")
            self.log(f"")
            ...
            if has_grasped and has_released:
                self.log(f"  ✅ Pick-place sequence completed (OPEN → CLOSE → OPEN)")
            elif has_grasped:
                self.log(f"  ⚠️  Gripper closed but did not release")
            elif trajectory_grippers.min() < gripper_close_threshold:
                self.log(f"  ⚠️  Gripper closed during episode")
            else:
                self.log(f"  ⚠️  Gripper stayed OPEN (all values > {gripper_close_threshold})")
            self.log(f"")
            ...
            if termination_reason:
                ...
                print(f"⏱️  Reached max_steps: {max_steps}")
...
def main():
    ...
    # Intelligent termination detection arguments
    parser.add_argument('--enable_gripper_termination', action='store_true', default=True,
                        help='Enable gripper-based task completion detection (default: True)')
    parser.add_argument('--disable_gripper_termination', action='store_false', dest='enable_gripper_termination',
                        help='Disable gripper-based termination')
    parser.add_argument('--enable_movement_termination', action='store_true', default=False,
                        help='Enable movement-based task completion detection (default: False)')
    parser.add_argument('--gripper_threshold', type=float, default=0.5,
                        help='Gripper close threshold (default: 0.5)')
    parser.add_argument('--movement_threshold', type=float, default=0.005,
                        help='Movement threshold in meters (default: 0.005 = 5mm)')
    parser.add_argument('--movement_window', type=int, default=10,
                        help='Number of steps to check for low movement (default: 10)')
    ...
    try:
        ...
        for episode in range(args.num_episodes):
            ...
            deployment.run_episode(
                ...
                enable_gripper_termination=args.enable_gripper_termination,
                enable_movement_termination=args.enable_movement_termination,
                gripper_close_threshold=args.gripper_threshold,
